...
| + | webpack 原理 | ✅2024/5/24 | webpack 分[3个阶段](https://mp.weixin.qq.com/s/SbJNbSVzSPSKBe2YStn2Zw)：初始化阶段、构建阶段、生成阶段。初始化阶段：处理process.args、webpack.config.js的用户参数，加载webpack需要的plugins插件。构建阶段：将JavaScript代码解析成 AST 对象，遍历出所需的依赖文件(引用的组件和图片)生成AST module对象集合。生成阶段：将获取到的module集合根据配置的entry规则生成chunks输出。 |
