...
| 27 | 手写防抖、节流 | ✅2024/6/20 | 这两种都属于性能优化，防抖：指在触发一次事件之后的一段时间内多次触发，也只执行最后那一次。实现思路：通过变量存setTimeout定时器执行业务逻辑，当多次触发时销毁前面的定时器，重新创建新的定时器。节流：指触发一次事件后的一段时间内多次触发，都只执行第一次点击触发的事件。实现思路：通过变量状态true控制if执行，执行后重置变量状态false。 |
...
| 35 | 你是怎么使用webworker的？ | - |
| 36 | 浏览器内存你在实战中处理过吗？ | - |
| 37 | 浏览器的垃圾回收机制是什么样的？ | ✅2024/6/28 | JavaScript不同于其他的语言，有专门的内存管理接口。JavaScript创建变量(字符串，对象)的时候就自动的进行分配内存，在不使用变量的时候就会自动释放内存。这个释放的过程就称为垃圾回收。至今为止垃圾回收机制有过2个版本，1、引用计数收集：当对象被引用的时候会给计数器+1，当引用被释放就-1，当对象没有被引用指向的时候为0就回收。缺点是循环引用不会被回收，容易导致内存泄漏。2、标记清除算法：机制会定时从全局对象开始，找到所有可以获得的对象并标记和收集所有不能获得的对象并清除。 |
| 38 | 微前端的理解 | - |
| 40 | 小程序跟H5的区别是什么？ | - |
