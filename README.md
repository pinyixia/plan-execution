...
| 23 | new一个对象的时候会做些什么？ | ✅2024/5/31 | 首先会创建一个__proto__原型链指向构造函数的prototype原型对象的新对象。2.然后将构造函数的this绑定到当前的新对象上。4.返回创建的新对象。不管是ES5的new继承还是ES6的extends关键字继承，本质上都是这种操作原型链机制。 |
...
| 25 | 跨域问题 | ✅2024/6/20 | 浏览器进行接口请求出现403状态码，有两种原因，一种是服务端设置了访问权限，另一种是因为请求的接口在不同的域下面导致浏览器内置的同源策略拦截了。先说什么情况下才会导致激发同源策略，请求的ip不同、端口不同、协议不同，这三种都会触发同源策略拦截。要解决这种情况，可以通过JSONP的方式，缺点就是只能进行GET请求。服务端响应配置cors同源将origin设置成*号，绕开同源策略的监控。通过服务代理，服务端是不存在跨域情况的。所以可以启动一个代理服务器。现在前端的脚手架都是基于node搭建的，node本身就是一个服务器，基本都有代理配置。不需要像以前需要后端配合代理。浏览器tab页之间的跨域共享可以通过window对象下postMessage方法来实现。另外由于服务端设置了访问权限导致的403访问失败，这种情况只能要求在开发阶段服务端放开访问权限。 |
...
| 66 | js原型链 | ✅2024/6/20 | 原型链：每个对象（object）都有一个私有属性指向另一个名为原型（prototype）的对象，这个私有属性[[Prototype]]可以通过getPrototypeOf访问，浏览器也都实现了更方便的访问器__proto__访问原型。原型对象也有一个自己的原型，层层向上直到一个对象的原型为 null。根据定义，null 没有原型，这个层层相连被称作原型链。对象继承：对象的属性都是动态的，当我们访问一个属性时，首先会对当前对象属性进行查找，当没找到时。就会通过原型链向原型对象进行向上依次层层查找，直至找到这个属性或者到达原型链末尾null。所以js的继承是通过使用构造函数基于原型链创造实例的方式实现的。我们创建的每一个实例都会自动将构造函数的 prototype 属性作为其 [[Prototype]] 原型，达到访问实例对象不存在的继承属性。另外每个构造函数的prototype对象都会默认创建一个constructor属性，引用的是构造函数本身。这个属性没什么特别的用处，甚至可以改它。 |
